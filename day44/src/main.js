...
function animate() {
  ...
  if(isHovered){
    ...
    const normalizedX = Math.abs(localPoint.x) / 0.75;
    const normalizedY = Math.abs(localPoint.y) / 1.5;
    const distanceFromCenter = Math.max(normalizedX, normalizedY);

    targetX = localPoint.x * distanceFromCenter * 0.02;
    targetY = localPoint.y * distanceFromCenter * 0.03;
    targetRotationX = localPoint.y * distanceFromCenter * 0.05;
    targetRotationY = -localPoint.x * distanceFromCenter * 0.05;
  }
